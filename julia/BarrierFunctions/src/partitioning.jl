...
domain(p::AbstractSemialgebraicSet) = p
...
struct IndexedEqualWidthPartitioning{T, L <: Integer} <: AbstractPartitioning
    variables::AbstractVector{<:MP.AbstractVariable}
    iterators::AbstractVector{LinRange{T, L}}

    function IndexedEqualWidthPartitioning(variables::AbstractVector{<:MP.AbstractVariable}, iterators::AbstractVector{LinRange{T, L}}) where {T, L<:Integer}
        N = length(variables)
        M = length(iterators)
        @assert(N == M, "Number of variables $N must match number of LinRanges $M")
...
IndexedEqualWidthPartitioning(variables::MP.AbstractVariable, iterators...) = IndexedEqualWidthPartitioning([variables], iterators...)
IndexedEqualWidthPartitioning(variables::Tuple{Vararg{<:MP.AbstractVariable}}, iterators...) = IndexedEqualWidthPartitioning(vec(variables), iterators...)

function IndexedEqualWidthPartitioning(variables::AbstractVector{<:MP.AbstractVariable}, iterators::Vararg{LinRange})
    return IndexedEqualWidthPartitioning(variables, collect(iterators))
end

function partition_iterator(partitioning::IndexedEqualWidthPartitioning)
    slice_iterators = product(map(_slice, partitioning.iterators)...)
    partition_iterator = map(_convert_to_vec, slice_iterators)

    semialgebraic_sets = map((p) -> _semialgebraic_set(partitioning.variables, p), partition_iterator)
    
    return enumerate(semialgebraic_sets)
end

domain(p::Tuple{<:Integer, AbstractSemialgebraicSet}) = p[2]