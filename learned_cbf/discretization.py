...
class BoundEuler(BoundModule):
    ...
    def crown_backward(self, linear_bounds):
        dt_bounds = LinearBounds(
            ...
            (self.module.dt * linear_bounds.lower[0], torch.zeros_like(linear_bounds.lower[1])) if linear_bounds.lower is not None else None,
            (self.module.dt * linear_bounds.upper[0], torch.zeros_like(linear_bounds.upper[1])) if linear_bounds.upper is not None else None
        )
...
class BoundRK4(BoundModule):
    ...
    def backward_relaxation(self, region):
        if self.bound_update1.need_relaxation:
            ...
            dt_bounds2 = LinearBounds(
                ...
                (self.module.dt * update_bounds2.lower[0] / 2, torch.zeros_like(update_bounds2.lower[1])),
                (self.module.dt * update_bounds2.upper[0] / 2, torch.zeros_like(update_bounds2.upper[1]))
            )

            update_bounds1 = self.bound_update1.crown_backward(dt_bounds2)
            return LinearBounds(
                update_bounds1.region,
                (update_bounds2.lower[0] + update_bounds1.lower[0], update_bounds2.lower[1] + update_bounds1.lower[1]),
                (update_bounds2.upper[0] + update_bounds1.upper[0], update_bounds2.upper[1] + update_bounds1.upper[1])
            ), relaxation_module
            ...
            return LinearBounds(
                update_bounds1.region,
                (update_bounds3.lower[0] + update_bounds1.lower[0] + update_bounds2.lower[0], update_bounds3.lower[1] + update_bounds1.lower[1] + update_bounds2.lower[1]),
                (update_bounds3.upper[0] + update_bounds1.upper[0] + update_bounds2.upper[0], update_bounds3.upper[1] + update_bounds1.upper[1] + update_bounds2.upper[1])
            ), relaxation_module
        else:
            ...
            dt_bounds4 = LinearBounds(
                ...
                (self.module.dt * update_bounds4.lower[0], torch.zeros_like(update_bounds4.lower[1])),
                (self.module.dt * update_bounds4.upper[0], torch.zeros_like(update_bounds4.upper[1]))
            )
            ...
            return LinearBounds(
                update_bounds1.region,
                (update_bounds4.lower[0] + update_bounds1.lower[0] + update_bounds2.lower[0] + update_bounds3.lower[0], update_bounds4.lower[1] + update_bounds1.lower[1] + update_bounds2.lower[1] + update_bounds3.lower[1]),
                (update_bounds4.upper[0] + update_bounds1.upper[0] + update_bounds2.upper[0] + update_bounds3.upper[0], update_bounds4.upper[1] + update_bounds1.upper[1] + update_bounds2.upper[1] + update_bounds3.upper[1])
            ), relaxation_module
    ...
    def crown_backward(self, linear_bounds):
        ...
        dt_bounds4 = LinearBounds(
            linear_bounds.region,
            (self.module.dt * linear_bounds.lower[0] / 6, zero_bias) if linear_bounds.lower is not None else None,
            (self.module.dt * linear_bounds.upper[0] / 6, zero_bias) if linear_bounds.upper is not None else None
        )
        update_bounds4 = self.bound_update3.crown_backward(dt_bounds4)

        dt_bounds3 = LinearBounds(
            linear_bounds.region,
            (self.module.dt * (linear_bounds.lower[0] / 3 + update_bounds4.lower[0]), zero_bias) if linear_bounds.lower is not None else None,
            (self.module.dt * (linear_bounds.upper[0] / 3 + update_bounds4.upper[0]), zero_bias) if linear_bounds.upper is not None else None
        )
        update_bounds3 = self.bound_update3.crown_backward(dt_bounds3)

        dt_bounds2 = LinearBounds(
            linear_bounds.region,
            (self.module.dt * (linear_bounds.lower[0] / 3 + update_bounds3.lower[0] / 2), zero_bias) if linear_bounds.lower is not None else None,
            (self.module.dt * (linear_bounds.upper[0] / 3 + update_bounds3.upper[0] / 2), zero_bias) if linear_bounds.upper is not None else None
        )
        update_bounds2 = self.bound_update2.crown_backward(dt_bounds2)

        dt_bounds1 = LinearBounds(
            linear_bounds.region,
            (self.module.dt * (linear_bounds.lower[0] / 6 + update_bounds2.lower[0] / 2), zero_bias) if linear_bounds.lower is not None else None,
            (self.module.dt * (linear_bounds.upper[0] / 6 + update_bounds2.upper[0] / 2), zero_bias) if linear_bounds.upper is not None else None
        )
        ...
        if linear_bounds.lower is None:
            ...
            lowerA = update_bounds1.lower[0] + update_bounds2.lower[0] + update_bounds3.lower[0] + update_bounds4.lower[0]
            lowerb = update_bounds1.lower[1] + update_bounds2.lower[1] + update_bounds3.lower[1] + update_bounds4.lower[1]
            lower = (linear_bounds.lower[0] + lowerA, linear_bounds.lower[1] + lowerb)

        if linear_bounds.upper is None:
            upper = None
        else:
            upperA = update_bounds1.upper[0] + update_bounds2.upper[0] + update_bounds3.upper[0] + update_bounds4.upper[0]
            upperb = update_bounds1.upper[1] + update_bounds2.upper[1] + update_bounds3.upper[1] + update_bounds4.upper[1]
            upper = (linear_bounds.upper[0] + upperA, linear_bounds.upper[1] + upperb)
...
class Heun(nn.Module):
    ...
    def forward(self, x):
        k1 = self.dt * self.update(x)
        k2 = self.dt * self.update(x + k1)

        return x + (k1 + k2) / 2


class BoundHeun(BoundModule):
    def __init__(self, module, factory, **kwargs):
        super().__init__(module, factory, **kwargs)
    ...
    @property
    def need_relaxation(self):
        return self.bound_update1.need_relaxation or self.bound_update2.need_relaxation
    ...
    def backward_relaxation(self, region):
        if self.bound_update1.need_relaxation:
            ...
            assert self.bound_update2.need_relaxation
            update_bounds2, relaxation_module = self.bound_update2.backward_relaxation(region)
            dt_bounds2 = LinearBounds(
                update_bounds2.region,
    ...
    def crown_backward(self, linear_bounds):
        ...
        dt_bounds2 = LinearBounds(
            linear_bounds.region,
            (self.module.dt * linear_bounds.lower[0] / 2, zero_bias) if linear_bounds.lower is not None else None,
            (self.module.dt * linear_bounds.upper[0] / 2, zero_bias) if linear_bounds.upper is not None else None
        )
        update_bounds2 = self.bound_update2.crown_backward(dt_bounds2)

        dt_bounds1 = LinearBounds(
            linear_bounds.region,
            (self.module.dt * (linear_bounds.lower[0] / 2 + update_bounds2.lower[0]), zero_bias) if linear_bounds.lower is not None else None,
            (self.module.dt * (linear_bounds.upper[0] / 2 + update_bounds2.upper[0]), zero_bias) if linear_bounds.upper is not None else None
        )
        update_bounds1 = self.bound_update1.crown_backward(dt_bounds1)
        ...
        if linear_bounds.lower is None:
            ...
            lowerA = update_bounds1.lower[0] + update_bounds2.lower[0]
            lowerb = update_bounds1.lower[1] + update_bounds2.lower[1]
            lower = (linear_bounds.lower[0] + lowerA, linear_bounds.lower[1] + lowerb)

        if linear_bounds.upper is None:
            upper = None
        else:
            upperA = update_bounds1.upper[0] + update_bounds2.upper[0]
            upperb = update_bounds1.upper[1] + update_bounds2.upper[1]
            upper = (linear_bounds.upper[0] + upperA, linear_bounds.upper[1] + upperb)
    ...
    def ibp_forward(self, bounds, save_relaxation=False):
        ...
        k2_input_bounds = IntervalBounds(bounds.region, bounds.lower + k1[0], bounds.upper + k1[1])
        k2 = self.bound_update2.ibp_forward(k2_input_bounds, save_relaxation=save_relaxation)
        k2 = self.module.dt * k2.lower, self.module.dt * k2.upper

        lower = bounds.lower + (k1[0] + k2[0]) / 2
        upper = bounds.upper + (k1[1] + k2[1]) / 2
        return IntervalBounds(bounds.region, lower, upper)
