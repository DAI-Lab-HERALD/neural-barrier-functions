...
def linear_main(args, config):
    logger.info('Constructing model')
    dynamics = LinearDynamics(config['dynamics']).to(args.device)
    ...
    if config['experiment_type'] == 'barrier_function':
        ...
        initial_partitioning = linear_partitioning(config, dynamics).to(args.device)
