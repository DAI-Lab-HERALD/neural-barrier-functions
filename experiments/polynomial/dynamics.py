...
class Polynomial(StochasticDynamics, nn.Module):
    ...
    def forward(self, x):
        x1 = x[..., 0] + self.dt * x[..., 1] + self.z
        x2 = x[..., 1] + self.dt * ((x[..., 0] ** 3) / 3.0 - x.sum(dim=-1)).unsqueeze(0).expand_as(x1)
...
class BoundPolynomial(BoundModule):
    ...
    def crown_backward(self, linear_bounds):
        assert self.bounded
        # TODO: Fix wrt. current state in Euler method (forgot about that before)
    ...
    def ibp_forward(self, bounds, save_relaxation=False):
        ...
        x1_lower = bounds.lower[..., 0] + self.module.dt * bounds.lower[..., 1] + self.module.z
        x1_upper = bounds.upper[..., 0] + self.module.dt * bounds.upper[..., 1] + self.module.z
        ...
        x2_lower = bounds.lower[..., 1] + self.module.dt * (x1_cubed_lower - bounds.upper.sum(dim=-1)).unsqueeze(0).expand_as(x1_lower)
        x2_upper = bounds.upper[..., 1] + self.module.dt * (x1_cubed_upper - bounds.lower.sum(dim=-1)).unsqueeze(0).expand_as(x1_lower)
