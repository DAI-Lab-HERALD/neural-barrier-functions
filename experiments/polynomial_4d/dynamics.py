...
class Polynomial(Euler, AdditiveGaussianDynamics):
    ...
    def initial(self, x, eps=None):
        ...
        return overlap_circle(lower_x, upper_x, torch.tensor([0.0, 0.0, 0.0, 0.0], device=x.device), math.sqrt(0.16))

    def sample_initial(self, num_particles):
        dist = distributions.Uniform(0, 1)
        r = math.sqrt(0.16) * dist.sample((num_particles,)).sqrt()
        theta = dist.sample((num_particles,)) * 2 * np.pi
        circle_samples = torch.stack([r * theta.cos(), r * theta.sin()], dim=-1)
    ...
    def safe(self, x, eps=None):
        ...
        return overlap_circle(lower_x, upper_x, torch.tensor([0.0, 0.0, 0.0, 0.0], device=x.device), math.sqrt(0.25))

    def sample_safe(self, num_particles):
        dist = distributions.Uniform(0, 1)
        r = math.sqrt(0.25) * dist.sample((num_particles,)).sqrt()
        theta = dist.sample((num_particles,)) * 2 * np.pi
        circle_samples = torch.stack([r * theta.cos(), r * theta.sin()], dim=-1)
    ...
    def unsafe(self, x, eps=None):
        ...
        return overlap_outside_circle(lower_x, upper_x, torch.tensor([0.0, 0.0, 0.0, 0.0], device=x.device), math.sqrt(0.25))
